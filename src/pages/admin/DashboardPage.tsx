    ...
    <div className="max-w-5xl mx-auto px-6 py-8">
      ...
      <div>
        <div className="flex items-center justify-between mb-4">
          ...
          <Link to="/admin/posts" className="text-xs text-emerald-500 hover:text-emerald-400 transition-colors">
            전체 보기 →
          </Link>
        </div>
        <div className="bg-zinc-900/60 border border-zinc-800/60 rounded-xl overflow-hidden">
          {recentPosts.map((post, i) => {
            const cat = post.categories as { name: string; emoji: string } | null
            return (
              <div
                key={post.id}
