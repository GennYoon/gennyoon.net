...
export default function AIWritingAssistant({ onGenerated, categories }: Props) {
  ...
  async function handleGenerate() {
    await complete(rawInput, {
      body: { rawInput, images, categorySlug: selectedCategory, tone },
    })
  }
  ...
  async function handleRefine(action: string) {
    ...
    try {
      const res = await fetch('/api/ai/refine', {
        ...
        body: JSON.stringify({ paragraph: selection, action }),
      })
      ...
      {completion && (
        <div className="flex-1 flex flex-col gap-3">
          ...
          {!isLoading && (
            <>
              <button
                onClick={() => onGenerated(completion)}
